...
async function extractTextFromPDF(file: File): Promise<string> {
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    if (pageText.trim().length < 50) {
      ...
      if (base64Image) {
        text += (await extractImageTextLocally(base64Image, i)) + '\n\n';
      }
    } else {
      text += pageText + '\n\n';
    }
...
export async function extractText(file: File): Promise<string> {
  ...
  if (['txt', 'csv', 'md', 'srt'].includes(ext || '')) {
  ...
  if (['png', 'jpg', 'jpeg'].includes(ext || '')) {
    ...
    if (base64Image) {
      return await extractImageTextLocally(base64Image, 'Image');
    }
    return `[Image skipped: Environment does not support image processing]`;
  }
  
  return `[Audio/Media skipped: Local parsing handles text/vision files only for ${file.name}]`;
}
