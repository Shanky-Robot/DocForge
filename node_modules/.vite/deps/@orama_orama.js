...
var internal_document_id_store_exports = /* @__PURE__ */ __exportAll({
...
var documents_store_exports = /* @__PURE__ */ __exportAll({
	count: () => count$1,
	create: () => create$4,
...
function insert$2(implementation, index, prop, id, internalId, value, schemaType, language, tokenizer, docsCount, options) {
...
function remove$2(implementation, index, prop, id, internalId, value, schemaType, language, tokenizer, docsCount) {
...
var sorter_exports = /* @__PURE__ */ __exportAll({
...
function remove$1(sorter, prop, id) {
	if (!sorter.enabled) return;
	const s = sorter.sorts[prop];
	const internalId = getInternalDocumentId(sorter.sharedInternalDocumentStore, id);
...
var tokenizer_exports = /* @__PURE__ */ __exportAll({
...
//#endregion
//#region node_modules/@orama/orama/dist/browser/components.js
...
function innerInsertMultiple(orama, docs, batchSize, language, skipHooks, timeout) {
	if (isAsyncFunction(orama.beforeInsert) || isAsyncFunction(orama.afterInsert) || isAsyncFunction(orama.index.beforeInsert) || isAsyncFunction(orama.index.insert) || isAsyncFunction(orama.index.afterInsert)) return innerInsertMultipleAsync(orama, docs, batchSize, language, skipHooks, timeout);
	return innerInsertMultipleSync(orama, docs, batchSize, language, skipHooks, timeout);
}
//#endregion
//#region node_modules/@orama/orama/dist/browser/methods/pinning.js
/**
...
//#endregion
//#region node_modules/@orama/orama/dist/browser/methods/remove.js
function remove(orama, id, language, skipHooks) {
	if (isAsyncFunction(orama.index.beforeRemove) || isAsyncFunction(orama.index.remove) || isAsyncFunction(orama.index.afterRemove)) return removeAsync(orama, id, language, skipHooks);
...
async function removeAsync(orama, id, language, skipHooks) {
	...
	const docsCount = orama.documentsStore.count(docs);
	if (!skipHooks) await runSingleHook(orama.beforeRemove, orama, docId);
	const indexableProperties = orama.index.getSearchableProperties(index);
	const indexablePropertiesWithTypes = orama.index.getSearchablePropertiesWithTypes(index);
	...
	for (const prop of indexableProperties) {
		...
		if (!await orama.index.remove(orama.index, orama.data.index, prop, id, internalId, value, schemaType, language, orama.tokenizer, docsCount)) result = false;
	...
	for (const prop of sortableProperties) {
		if (typeof sortableValues[prop] === "undefined") continue;
		orama.sorter.remove(orama.data.sorting, prop, id);
	}
	if (!skipHooks) await runSingleHook(orama.afterRemove, orama, docId);
...
function removeSync(orama, id, language, skipHooks) {
	...
	const docId = getDocumentIdFromInternalId(orama.internalDocumentIDStore, internalId);
	const docsCount = orama.documentsStore.count(docs);
	if (!skipHooks) runSingleHook(orama.beforeRemove, orama, docId);
	const indexableProperties = orama.index.getSearchableProperties(index);
	const indexablePropertiesWithTypes = orama.index.getSearchablePropertiesWithTypes(index);
	const values = orama.getDocumentProperties(doc, indexableProperties);
	for (const prop of indexableProperties) {
		...
		if (!orama.index.remove(orama.index, orama.data.index, prop, id, internalId, value, schemaType, language, orama.tokenizer, docsCount)) result = false;
	...
	if (!skipHooks) runSingleHook(orama.afterRemove, orama, docId);
	orama.documentsStore.remove(orama.data.docs, id, internalId);
	return result;
}
function removeMultiple(orama, ids, batchSize, language, skipHooks) {
	if (isAsyncFunction(orama.index.beforeRemove) || isAsyncFunction(orama.index.remove) || isAsyncFunction(orama.index.afterRemove) || isAsyncFunction(orama.beforeRemoveMultiple) || isAsyncFunction(orama.afterRemoveMultiple)) return removeMultipleAsync(orama, ids, batchSize, language, skipHooks);
...
async function removeMultipleAsync(orama, ids, batchSize, language, skipHooks) {
	...
	if (!skipHooks) await runMultipleHook(orama.beforeRemoveMultiple, orama, docIdsForHooks);
	await new Promise((resolve, reject) => {
		let i = 0;
		async function _removeMultiple() {
			const batch = ids.slice(i * batchSize, ++i * batchSize);
			if (!batch.length) return resolve();
			for (const doc of batch) try {
				if (await remove(orama, doc, language, skipHooks)) result++;
			} catch (err) {
				reject(err);
			}
	...
	if (!skipHooks) await runMultipleHook(orama.afterRemoveMultiple, orama, docIdsForHooks);
	return result;
}
function removeMultipleSync(orama, ids, batchSize, language, skipHooks) {
	...
	if (!batchSize) batchSize = 1e3;
	const docIdsForHooks = skipHooks ? [] : ids.map((id) => getDocumentIdFromInternalId(orama.internalDocumentIDStore, getInternalDocumentId(orama.internalDocumentIDStore, id)));
	if (!skipHooks) runMultipleHook(orama.beforeRemoveMultiple, orama, docIdsForHooks);
	let i = 0;
	function _removeMultipleSync() {
		const batch = ids.slice(i * batchSize, ++i * batchSize);
		if (!batch.length) return;
		for (const doc of batch) if (remove(orama, doc, language, skipHooks)) result++;
		setTimeout(_removeMultipleSync, 0);
	}
	_removeMultipleSync();
	if (!skipHooks) runMultipleHook(orama.afterRemoveMultiple, orama, docIdsForHooks);
	return result;
}
//#endregion
//#region node_modules/@orama/orama/dist/browser/constants.js
...
//#endregion
//#region node_modules/@orama/orama/dist/browser/methods/update.js
function update(orama, id, doc, language, skipHooks) {
	if (isAsyncFunction(orama.afterInsert) || isAsyncFunction(orama.beforeInsert) || isAsyncFunction(orama.afterRemove) || isAsyncFunction(orama.beforeRemove) || isAsyncFunction(orama.beforeUpdate) || isAsyncFunction(orama.afterUpdate)) return updateAsync(orama, id, doc, language, skipHooks);
	return updateSync(orama, id, doc, language, skipHooks);
}
async function updateAsync(orama, id, doc, language, skipHooks) {
	if (!skipHooks && orama.beforeUpdate) await runSingleHook(orama.beforeUpdate, orama, id);
	await remove(orama, id, language, skipHooks);
	const newId = await insert(orama, doc, language, skipHooks);
	if (!skipHooks && orama.afterUpdate) await runSingleHook(orama.afterUpdate, orama, newId);
	return newId;
}
function updateSync(orama, id, doc, language, skipHooks) {
	if (!skipHooks && orama.beforeUpdate) runSingleHook(orama.beforeUpdate, orama, id);
	remove(orama, id, language, skipHooks);
	const newId = insert(orama, doc, language, skipHooks);
	if (!skipHooks && orama.afterUpdate) runSingleHook(orama.afterUpdate, orama, newId);
	return newId;
}
function updateMultiple(orama, ids, docs, batchSize, language, skipHooks) {
	if (isAsyncFunction(orama.afterInsert) || isAsyncFunction(orama.beforeInsert) || isAsyncFunction(orama.afterRemove) || isAsyncFunction(orama.beforeRemove) || isAsyncFunction(orama.beforeUpdate) || isAsyncFunction(orama.afterUpdate) || isAsyncFunction(orama.beforeUpdateMultiple) || isAsyncFunction(orama.afterUpdateMultiple) || isAsyncFunction(orama.beforeRemoveMultiple) || isAsyncFunction(orama.afterRemoveMultiple) || isAsyncFunction(orama.beforeInsertMultiple) || isAsyncFunction(orama.afterInsertMultiple)) return updateMultipleAsync(orama, ids, docs, batchSize, language, skipHooks);
...
async function updateMultipleAsync(orama, ids, docs, batchSize, language, skipHooks) {
	if (!skipHooks) await runMultipleHook(orama.beforeUpdateMultiple, orama, ids);
	...
	if (!skipHooks) await runMultipleHook(orama.afterUpdateMultiple, orama, newIds);
	return newIds;
}
function updateMultipleSync(orama, ids, docs, batchSize, language, skipHooks) {
	if (!skipHooks) runMultipleHook(orama.beforeUpdateMultiple, orama, ids);
	const docsLength = docs.length;
	for (let i = 0; i < docsLength; i++) {
		const errorProperty = orama.validateSchema(docs[i], orama.schema);
		if (errorProperty) throw createError("SCHEMA_VALIDATION_FAILURE", errorProperty);
	...
	if (!skipHooks) runMultipleHook(orama.afterUpdateMultiple, orama, newIds);
	return newIds;
}
//#endregion
//#region node_modules/@orama/orama/dist/browser/methods/upsert.js
function upsert(orama, doc, language, skipHooks, options) {
	if (isAsyncFunction(orama.afterInsert) || isAsyncFunction(orama.beforeInsert) || isAsyncFunction(orama.afterRemove) || isAsyncFunction(orama.beforeRemove) || isAsyncFunction(orama.beforeUpdate) || isAsyncFunction(orama.afterUpdate) || isAsyncFunction(orama.beforeUpsert) || isAsyncFunction(orama.afterUpsert) || isAsyncFunction(orama.index.beforeInsert) || isAsyncFunction(orama.index.insert) || isAsyncFunction(orama.index.afterInsert)) return upsertAsync(orama, doc, language, skipHooks, options);
	return upsertSync(orama, doc, language, skipHooks, options);
}
async function upsertAsync(orama, doc, language, skipHooks, options) {
	const id = orama.getDocumentIndexId(doc);
	if (typeof id !== "string") throw createError("DOCUMENT_ID_MUST_BE_STRING", typeof id);
	if (!skipHooks && orama.beforeUpsert) await runSingleHook(orama.beforeUpsert, orama, id, doc);
	const existingDoc = orama.documentsStore.get(orama.data.docs, id);
	let resultId;
	if (existingDoc) resultId = await update(orama, id, doc, language, skipHooks);
	else resultId = await insert(orama, doc, language, skipHooks, options);
	if (!skipHooks && orama.afterUpsert) await runSingleHook(orama.afterUpsert, orama, resultId, doc);
	return resultId;
}
function upsertSync(orama, doc, language, skipHooks, options) {
	const id = orama.getDocumentIndexId(doc);
	if (typeof id !== "string") throw createError("DOCUMENT_ID_MUST_BE_STRING", typeof id);
	if (!skipHooks && orama.beforeUpsert) runSingleHook(orama.beforeUpsert, orama, id, doc);
	const existingDoc = orama.documentsStore.get(orama.data.docs, id);
	let resultId;
	if (existingDoc) resultId = update(orama, id, doc, language, skipHooks);
	else resultId = insert(orama, doc, language, skipHooks, options);
	if (!skipHooks && orama.afterUpsert) runSingleHook(orama.afterUpsert, orama, resultId, doc);
	return resultId;
}
function upsertMultiple(orama, docs, batchSize, language, skipHooks) {
	if (isAsyncFunction(orama.afterInsert) || isAsyncFunction(orama.beforeInsert) || isAsyncFunction(orama.afterRemove) || isAsyncFunction(orama.beforeRemove) || isAsyncFunction(orama.beforeUpdate) || isAsyncFunction(orama.afterUpdate) || isAsyncFunction(orama.beforeUpsert) || isAsyncFunction(orama.afterUpsert) || isAsyncFunction(orama.beforeUpsertMultiple) || isAsyncFunction(orama.afterUpsertMultiple) || isAsyncFunction(orama.beforeInsertMultiple) || isAsyncFunction(orama.afterInsertMultiple) || isAsyncFunction(orama.beforeUpdateMultiple) || isAsyncFunction(orama.afterUpdateMultiple) || isAsyncFunction(orama.beforeRemoveMultiple) || isAsyncFunction(orama.afterRemoveMultiple) || isAsyncFunction(orama.index.beforeInsert) || isAsyncFunction(orama.index.insert) || isAsyncFunction(orama.index.afterInsert)) return upsertMultipleAsync(orama, docs, batchSize, language, skipHooks);
...
async function upsertMultipleAsync(orama, docs, batchSize, language, skipHooks) {
	if (!skipHooks && orama.beforeUpsertMultiple) await runMultipleHook(orama.beforeUpsertMultiple, orama, docs);
	const docsLength = docs.length;
	for (let i = 0; i < docsLength; i++) {
		const errorProperty = orama.validateSchema(docs[i], orama.schema);
		if (errorProperty) throw createError("SCHEMA_VALIDATION_FAILURE", errorProperty);
	...
	for (const doc of docs) {
		const id = orama.getDocumentIndexId(doc);
		if (typeof id !== "string") throw createError("DOCUMENT_ID_MUST_BE_STRING", typeof id);
		if (orama.documentsStore.get(orama.data.docs, id)) {
			docsToUpdate.push(doc);
			idsToUpdate.push(id);
		} else docsToInsert.push(doc);
	}
	const results = [];
	...
	if (!skipHooks && orama.afterUpsertMultiple) await runMultipleHook(orama.afterUpsertMultiple, orama, results);
	return results;
}
function upsertMultipleSync(orama, docs, batchSize, language, skipHooks) {
	if (!skipHooks && orama.beforeUpsertMultiple) runMultipleHook(orama.beforeUpsertMultiple, orama, docs);
	const docsLength = docs.length;
	for (let i = 0; i < docsLength; i++) {
		const errorProperty = orama.validateSchema(docs[i], orama.schema);
		if (errorProperty) throw createError("SCHEMA_VALIDATION_FAILURE", errorProperty);
	...
	if (!skipHooks && orama.afterUpsertMultiple) runMultipleHook(orama.afterUpsertMultiple, orama, results);
	return results;
}
//#endregion
//#region node_modules/@orama/orama/dist/browser/methods/answer-session.js
...
var AnswerSession = class {
	...
	async ask(query) {
		...
		let output = "";
		for await (const msg of await this.askStream(query)) output += msg;
		return output;
	}
	...
	regenerateLast({ stream = true }) {
		...
		if (!(this.messages.at(-1)?.role === "assistant")) throw createError("ANSWER_SESSION_LAST_MESSAGE_IS_NOT_ASSISTANT");
		this.messages.pop();
		this.state.pop();
		if (stream) return this.askStream(this.lastInteractionParams);
		return this.ask(this.lastInteractionParams);
	}
	async *fetchAnswer(params) {
		if (!this.chatModel) throw createError("PLUGIN_SECURE_PROXY_MISSING_CHAT_MODEL");
		...
		try {
			...
			if (err.name === "AbortError") this.state[stateIdx].aborted = true;
			else {
	...
	triggerStateChange() {
		if (this.events.onStateChange) this.events.onStateChange(this.state);
	}
	async init() {
		const self = this;
		...
		if (!plugin) throw createError("PLUGIN_SECURE_PROXY_NOT_FOUND");
		...
		if (pluginExtras?.pluginParams?.chat?.model) this.chatModel = pluginExtras.pluginParams.chat.model;
		else throw createError("PLUGIN_SECURE_PROXY_MISSING_CHAT_MODEL");
	}
...
//#endregion
//#region node_modules/@orama/orama/dist/browser/types.js
var kInsertions = Symbol("orama.insertions");
var kRemovals = Symbol("orama.removals");
//#endregion
//#region node_modules/@orama/orama/dist/browser/internals.js
...
//#endregion
export { AnswerSession, MODE_FULLTEXT_SEARCH, MODE_HYBRID_SEARCH, MODE_VECTOR_SEARCH, components_exports as components, count, create, deletePin, getAllPins, getByID, getPin, insert, insertMultiple, insertPin, internals_exports as internals, kInsertions, kRemovals, load, remove, removeMultiple, save, search, searchVector, update, updateMultiple, updatePin, upsert, upsertMultiple };
